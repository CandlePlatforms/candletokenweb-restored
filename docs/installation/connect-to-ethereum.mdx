...
`candle` requires access to an [Ethereum](https://ethereum.org/en/) network for
typical usage.
...
pricing plans. That being said, the latest versions of `candle` should be able
...
this time `candle` has been the most thoroughly tested with `geth`.
...
2. Sync `geth`. The following command will sync `geth` on mainnet:
   ...
   geth \
       -rpc \
       -rpcapi eth,net,web3 \
       -rpcaddr <ADDRESS> # An address to bind on is required if geth is running on a different machine than candle
   ```
...
4. Connect `candle` to `geth` with the following flags (all other flags
   omitted):

   ```bash
   candle \
       -network mainnet \
       -ethUrl http://localhost:8545 # Assumes that geth is running on the same machine as candle
   ```

## Supported networks

`candle` supports the networks listed below. The required flags for connecting
...
Custom private networks where the Candle smart contracts are deployed can be
...
Offchain networks that do not require interaction with the Candle smart
