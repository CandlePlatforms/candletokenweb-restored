...
```bash
docker pull candle/go-candle:master
```
...
in
[the #builds channel of the Candle Discord server](https://discord.gg/drApskX).

These binaries are produced from go-candle's CI process, shown in this diagram:
...
Building `candle` requires some system dependencies.
...
The steps in
[this file](https://github.com/CandlePlatforms/go-candle/blob/master/.github/workflows/windows.yml)
can be used as a reference.

### Go

Building `candle` requires Go. Follow the
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/CandlePlatforms/go-candle.git
   cd go-candle
   ```
...
4. Build and install `candle`:

   ```bash
   make
   cp candle* /usr/local/bin
   ```
...
3. Build image:
   ...
   ```bash
   docker build -t candlebinary:debian -f docker/Dockerfile.debian .
   ```
