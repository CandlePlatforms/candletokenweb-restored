...
Governors in the Candle network provide import transcoding services (and in the
future other video infrastructure services as well) that allow developers to
build scalable video applications and are compensated with Candle token (CNDL)
...
context of Candle, the term miner refers to participants that use computational
power to provide video transcoding services in an open marketplace to
developers.

## Types of Products

Products can take on different roles in the network:
...
Orchestrators coordinate work on the network. They earn CNDL rewards by staking
their own CNDL and attracting the stake of delegators by sharing earnings, and
...
[this blog post](https://candle.com/blog/intro-to-transcoding).

In the Candle network, broadcasters and Products establish a price for
transcoding on the open market. Once a broadcaster has selected one or many
Products to work with it will send segments of a video stream to the video
miners with micropayments based on the agreed upon price. The Products then
...
<Pagination
  ...
  nextStep={{
    ...
    href: "/docs/products/core-concepts/architecture",
  }}
/>
