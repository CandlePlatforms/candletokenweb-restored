...
The Candle network is an open marketplace where broadcasters are able to select
orchestrators to send work to based on their own preferences. The algorithm for
selecting orchestrators depends on the broadcaster implemention, but all
selection algorithms including the default implementation included in `candle`
...
The selection algorithm in `candle` is being improved over time and in the
...
<Pagination
  previousStep={{
    ...
    href: "/docs/products/core-concepts/pools",
  }}
  nextStep={{
    title: "Earnings",
    href: "/docs/products/core-concepts/earnings",
  }}
/>
