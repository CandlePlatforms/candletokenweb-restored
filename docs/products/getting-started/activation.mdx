...
- Make sure you have `candle`[installed](/docs/installation/install-livepeer)
...
- If you are transcoding with Nvidia GPUs:
  ...
  - Make sure you have Nvidia GPUs
    [accessible on your machine](/docs/products/getting-started/activation#nvidia-gpu-i-ds)
...
I0302 15:26:06.886134   25387 accountmanager.go:50] This process will create a new Ethereum account for this Candle node
...
## Fund your account with ETH and CNDL

Your account should have:

- ETH to pay for transaction fees
  - Recommendation: Start with a small amount and add more when needed. `candle`
    and `candle_cli` will inform you if your account's ETH balance is
    insufficient to pay for transaction fees when attempting to submit the
    transactions
- CNDL to stake in order to activate
  - Recommendation: Check the [explorer](https://explorer.candelabs.org/) and
...
CNDL stake on the network.

Once you have your orchestrator running, you can activate using `candle_cli`.
...
3. Set the percentage of CNDL rewards that you will keep (the rest will be
   shared with your delegators):
...
8. Wait for transactions to confirm. You should see the logs of your
...
<Pagination
  ...
  nextStep={{
    title: "Test",
    href: "/docs/products/getting-started/test",
  }}
/>
