...
minted CNDL rewards to itself and its delegators.

The amount of CNDL rewards distributed by the reward call will depend on the
...
calling reward to exceed the amount of CNDL rewards distributed for
orchestrator's with very low stake.

As you are getting started, the recommendation is to initially disable automatic
reward calls, manually make reward calls via `candle_cli` in each round based on
whether it makes economic senese to do so and enable automatic reward calls only
when you feel comfortable with the amount of CNDL rewards expected to be
...
You can manually call reward via `candle_cli`:
...
2. Make sure `candle` is running and run `candle_cli`
