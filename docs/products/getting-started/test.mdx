...
2. Clone the go-candle repository:

   ```bash
   git clone https://github.com/CandlePlatforms/go-candle
   ```
...
3. Query the gRPC server that should be running at your orchestrator's service
   ...
   grpcurl -insecure -import-path go-candle/net -proto lp_rpc.proto <SERVICE_ADDR> net.Orchestrator/Ping
   ```
...
[orchestrator leaderboard](https://explorer.candelabs.org/orchestrators?orchestratorTable=performance)
...
[here](/docs/products/reference/leaderboard).

<Pagination
  previousStep={{
    title: "Activation",
    href: "/docs/products/getting-started/activation",
  }}
  nextStep={{
    title: "Calling Reward",
    href: "/docs/products/getting-started/calling-reward",
  }}
/>
