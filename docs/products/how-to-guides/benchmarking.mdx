...
- Make sure `candle_bench` is installed by following the
  [installation guide](/docs/installation/install-candle). `candle_bench` is
...
[common output rendition configuration](https://github.com/CandlePlatforms/go-candle/blob/master/cmd/candle_bench/transcodingOptions.json).
...
```bash
*---------------------*----------------------------------------------*
| Source File         | .../go-candle/bbb/source.m3u8              |
...
[session limits guide](/docs/products/how-to-guides/session-limits) for more
details.
