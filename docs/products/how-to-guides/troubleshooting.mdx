...
This page contains troubleshooting advice for Products and lists some of the
...
[session limit guide](/docs/products/how-to-guides/session-limits) for
...
that are supported by the Candle network.
...
Orchestrators will not be able to serve the Candle network if they are behind a
NAT (eg, a home router). If this is the case, special accommodations must be
...
If you set the `-v 6` flag when starting `candle`, more verbose logs indicating
transcoding activity will be available. You can also setup
[metrics monitoring](/docs/products/how-to-guides/metrics-monitoring).

**How do I keep a record of my node's logs?**

By default, `candle` will only send logs to stdout so they can be shown in your
terminal. However, a tool such as
[tee](https://linuxize.com/post/linux-tee-command/) can be used to pipe the logs
to both stdout and a log file.

```bash
candle ... 2>&1 | tee candle.log
```

<Pagination
  previousStep={{
    title: "Vote",
    href: "/docs/products/how-to-guides/vote",
  }}
/>
