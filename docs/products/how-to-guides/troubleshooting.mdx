...
Orchestrators will not be able to serve the Candle network if they are behind
...
If you set the `-v 6` flag when starting `candle`, more verbose logs
...
By default, `candle` will only send logs to stdout so they can be shown in
