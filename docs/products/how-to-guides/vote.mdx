...
## Vote with candle_cli

Orchestrators can use `candle_cli` to vote in governance polls without exporting
their keys from the machine that their orchestrator node is running on.
...
1. Find the contract address for the poll in the explorer page for the poll.
   ...
   ![CandleCLIVoteCTA](/docs-assets/vote-livepeer-cli.png)

   Click the "Follow the instructions" link to display the instructions for
   voting with `candle_cli`:

   ![CandleCLIVoteInstructions](/docs-assets/vote-livepeer-cli-instructions.png)
