...
The Candle Protocol specification lays out the various roles in the Candle
...
a high level summary, users that participate in the Candle protocol have one of
following roles:
...
They stake CNDL to secure the work that they perform, and ensure it is done
...
Candle protocol aware, it has no requirement for high reliability or being
...
the Candle network, it takes input streams from the end-user on its exposed RTMP
interface to have them transcoded by the infrastructure providers running on
Candle. The broadcaster takes care of splitting up streams into segments for
transcoding and aggregating the transcoded results in a media playlist.
Broadcasters are able to determine the output renditions and maximum price per
pixel for transcoding jobs it sends into the Candle network and pay for these
jobs in ETH using
[probabilistic micropayments](https://medium.com/@candlelabs/streamflow-probabilistic-micropayments-f3a647672462).

## Delegator

A **delegator** is a Candle tokenholder who participates in the network by
...
The Candle core team along with the broader Candle community drives development
...
<Pagination
  ...
  nextStep={{
    title: "Candle Token",
    href: "/docs/protocol/core-concepts/token",
  }}
/>
