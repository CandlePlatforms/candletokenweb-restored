...
a high level summary, users that participate in the Candle protocol have one
of following roles:
...
the Candle network, it takes input streams from the end-user on its exposed
RTMP interface to have them transcoded by the infrastructure providers running
on Candle. The broadcaster takes care of splitting up streams into segments
...
The Candle core team along with the broader Candle community drives
