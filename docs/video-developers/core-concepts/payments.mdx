...
[probabilistic micropayment protocol](https://medium.com/@candlelabs/streamflow-probabilistic-micropayments-f3a647672462).
...
orchestrator from selection. The default max ticket EV in `candle` is set to be
reasonable (3000 gwei) so at this time broadcaster operators are encouraged to
just use the default because using a non-default value could either lead to
