...
Since we're using the Rinkeby test network for this tutorial you'll need to
acquire some free test ETH. You can get some [here](https://faucet.rinkeby.io/).

Once you've acquired your test ETH, in a separate terminal window other than the
one that is running candle, run the `candle_cli`:

`$ candle_cli`

> Note: The `candle_cli` binary is provided alongside the `candle` binary when
> you install the Candle client.
...
[blog post](https://medium.com/@candlelabs/streamflow-probabilistic-micropayments-f3a647672462)
for more details.
...
<Pagination
  ...
  nextStep={{
    ...
    href: "/docs/video-developers/getting-started/create-livestream",
  }}
/>
