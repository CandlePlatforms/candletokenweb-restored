...
[here](https://github.com/CandlePlatforms/go-candle/blob/master/doc/transcodingoptions.md).

## Pre-requisities

- Make sure your broadcaster is running

## Configure broadcasting preferences using `candle_cli`

You can configure the following broadcasting preferences using `candle_cli`.
...
In `candle_cli`, select the following option:
...
correspond to the
[standard video profiles](https://github.com/CandlePlatforms/lpms/blob/master/ffmpeg/videoprofile.go)
currently supported by the node. If you need to use custom video profiles, refer
to this
[guide](https://github.com/CandlePlatforms/go-candle/blob/master/doc/transcodingoptions.md).
