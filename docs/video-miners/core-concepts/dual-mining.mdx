...
hashing algorithms using the same GPU. In the context of Candle, dual mining
can still involve using a GPU to mine a cryptocurrency like Ethereum, but it
also involves using the GPU to transcode video as a video miner on the Candle
network.
...
- Improve the utilization of GPUs
  - This can be of particular interest to Candle video miners since unlike in
...
- There is still a performance tradeoff between the two workloads. For example,
  ...
  [dual mining guide](/docs/products/how-to-guides/dual-mining) for more
  information
...
<Pagination
  previousStep={{
    title: "Earnings",
    href: "/docs/products/core-concepts/earning",
  }}
  nextStep={{
    title: "Payments",
    href: "/docs/products/core-concepts/payments",
  }}
/>
